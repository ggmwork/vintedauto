...
function getConfiguredTaskProvider(task: AiTask): AiProvider {
  ...
  return parseProvider(taskKey) ?? getLegacyProvider();
}
...
function getGenericTaskModel(task: AiTask) {
  return task === "listing"
    ? process.env.AI_LISTING_MODEL?.trim() || null
    : process.env.AI_GROUPING_MODEL?.trim() || null;
}

export function getAiRouterMode(): AiRouterMode {
  return parseRouterMode(process.env.AI_ROUTER_MODE);
}
...
export function getProviderTimeoutMs(provider: AiProvider, task: AiTask) {
  switch (provider) {
    case "ollama":
      return parseTimeout(process.env.OLLAMA_TIMEOUT_MS, task === "listing" ? 300_000 : 120_000);
    case "openai":
      return parseTimeout(process.env.OPENAI_TIMEOUT_MS, 120_000);
    case "anthropic":
      return parseTimeout(process.env.ANTHROPIC_TIMEOUT_MS, 120_000);
  }
}
